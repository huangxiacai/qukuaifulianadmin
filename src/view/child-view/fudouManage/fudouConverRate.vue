...
<script>
  ...
  import editfudouTransfer from './components/editfudouTransfer'
  export default {
    ...
    data () {
      return {
        ...
        columnsheader: [
          ...
          {
            title: '原始类型id',
            key: 'originBeanTypeId',
            align: 'center',
          },
          {
            title: '转换类型',
            key: 'transferBeanName',
            align: 'center'
          },
          {
            title: '转换类型Id',
            key: 'transferBeanTypeId',
            align: 'center',
          },
          {
            title: '转换汇率',
    ...
    methods: {
      ...mapActions([
        'handlequeryBeanTransferRates',
        'handleupdateBeanTransferRate'
      ]),
      //
      showSearchPanel () {
      ...
      edit(row){
        ...
        let config = {
          ...
          onOk: function () {
            let _this = this;
            let obj = this.$refs.editfudouTransfer;
            obj.checkForm().then(res => {
              if (res) {
                let getData = obj.getData;
                // 发送请求
                vm.handleupdateBeanTransferRate({
                  ...getData
                }).then(res => {
                  if (res.code === 20000) {
                    vm.$Message.success('修改福袋转汇率成功！');
                    vm.$Modal.remove();
                    vm.init()
                  } else {
                    vm.$Message.error(res.msg);
      ...
      init () {
        ...
        this.handlequeryBeanTransferRates({ ...this.reqBase }).then(res => {
